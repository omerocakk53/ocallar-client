...
function DialogContent({ className, children, showCloseButton = true, title = '', ...props }) {
  return (
    <DialogPortal data-slot="dialog-portal">
      ...
      <DialogPrimitive.Content
        ...
        className={cn(
          'bg-white data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 fixed top-[50%] left-[50%] z-50 grid w-full max-w-[calc(100%-2rem)] translate-x-[-50%] translate-y-[-50%] gap-4 rounded-lg border border-neutral-200 shadow-lg duration-200 sm:max-w-lg dark:bg-neutral-950 dark:border-neutral-800',
          'max-h-[calc(100vh-6rem)] h-full',
          className,
        )}
        {...props}
      >
        ...
        <div className={cn('h-full overflow-y-auto px-6 pb-6', !title && 'pt-6')}>{children}</div>
        {showCloseButton && (
          <DialogPrimitive.Close
            data-slot="dialog-close"
